...
function ENT:Splode()
	if(!self.Exploded) then
		...
		SB.util.damage.doBlastDamage( self.Entity:GetPos(), 1000, math.random(5000,10000), 7)
		local targets = ents.FindInSphere( self.Entity:GetPos(), 1000)
	
		for _,i in pairs(targets) do
			if i:GetClass() == "prop_physics" then
				i:GetPhysicsObject():ApplyForceOffset( Vector(500000,500000,500000), self.Entity:GetPos() )			end
		end
