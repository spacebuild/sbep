...
function ENT:MakeWire(bAdjust) --Adds the appropriate wire inputs.
    ...
    if bAdjust then
        self.Inputs = Wire_AdjustInputs(self, self.SBEP_WireInputsTable)
    else
        self.Inputs = Wire_CreateInputs(self, self.SBEP_WireInputsTable)
    end

    self.Outputs = WireLib.CreateOutputs(self, { "Floor", "Holding" })
end
