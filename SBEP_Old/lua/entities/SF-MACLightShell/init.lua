...
function ENT:Initialize()
	...
	self:SetModel( "models/Slyfo/nacshortsright.mdl" )
	...
	util.SpriteTrail( self, 0,  Color(50,50,200,200), false, 50, 0, 3, 1, "trails/smoke.vmt" )
...
function ENT:Think()
	...
	if (self.DTimer == true) then
		if (CurTime() >= self.DTime) then
			util.BlastDamage(self, self, self:GetPos(), 600, 100)
			cbt_hcgexplode( self:GetPos(), 400, math.random(1000,2000), 5)
 			local targets = ents.FindInSphere( self:GetPos(), 700)
	...
	if(!self.Exploded) then
		...
		if (data.HitEntity:IsWorld() or data.HitEntity:IsPlayer() or data.HitEntity:IsNPC()) then
			...
			if (data.HitEntity ~= self.LastHit) then
				local attack = cbt_dealhcghit( data.HitEntity, 1500, self.PStr, self:GetPos() , self:GetPos())
				if (attack == 0) then
					self.Exploded = true
					util.BlastDamage(self, self, self:GetPos(), 500, 100)
					cbt_hcgexplode( self:GetPos(), 400, math.random(1000,1500), 2)
