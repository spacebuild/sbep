...
function ENT:Draw()
	...
	if self.EfPoints and table.getn(self.EfPoints) > 0 then
		...
		local DMode = self:GetNWInt("DMode")
		...
		if DMode == 2 or DMode == 3 then
			for x = 1,table.getn(self.EfPoints),1 do
				...
				render.DrawSprite( self:GetPos() + self:GetRight() * self.EfPoints[x].x + self:GetForward() * self.EfPoints[x].y + self:GetUp() * self.EfPoints[x].z, 20, 20, color )
				...
				--print(Scroll)
				render.DrawBeam( self:GetPos() + self:GetRight() * self.EfPoints[x].x + self:GetForward() * self.EfPoints[x].y + self:GetUp() * self.EfPoints[x].z, self:GetPos() + self:GetRight() * self.EfPoints[NP].x + self:GetForward() * self.EfPoints[NP].y + self:GetUp() * self.EfPoints[NP].z, Sz, Scroll + 10, Scroll, Color( 255, 255, 255, 255 ) ) 
			end
		end
		if DMode == 3 then
			local LinkLock = self:GetNetworkedEntity( "LinkLock" )
			if LinkLock and LinkLock:IsValid() then
				for x = 1,table.getn(self.EfPoints),1 do
					...
					if NP ~= 0 then
						...
						render.DrawBeam( self:GetPos() + self:GetRight() * self.EfPoints[x].x + self:GetForward() * self.EfPoints[x].y + self:GetUp() * self.EfPoints[x].z, LinkLock:GetPos() + LinkLock:GetRight() * LinkLock.EfPoints[NP].x + LinkLock:GetForward() * LinkLock.EfPoints[NP].y + LinkLock:GetUp() * LinkLock.EfPoints[NP].z, 10, Scroll + 10, Scroll, Color( 255, 255, 255, 255 ) ) 
					end
				end
			end
		end
...
function ENT:Think()
	for i = 1,10 do
		...
		if Vec and Vec ~= Vector(0,0,0) then
			...
			self.EfPoints[i].sp = self:GetNetworkedInt("EfSp"..i) or 0
		end
	end
