...
function ENT:Initialize()
	...
	self:SetModel( "models/Slyfo/side_pod.mdl" )
...
function ENT:Think()
	if (self.NPod1 == nil or !self.NPod1:IsValid() or self.NPod1.Active) then
		...
		self.NPod1:SetAngles( (self:GetRight()*-1):Angle() )
		self.NPod1:SetPos( self:LocalToWorld(Vector(120,200,-45)) )
	...
	if (self.NPod2 == nil or !self.NPod2:IsValid() or self.NPod2.Active) then
		...
		self.NPod2:SetAngles( (self:GetRight()*1):Angle() )
		self.NPod2:SetPos( self:LocalToWorld(Vector(120,-200,-45)) )
	...
	if (self.NPod3 == nil or !self.NPod3:IsValid() or self.NPod3.Active) then
		...
		self.NPod3:SetAngles( (self:GetRight()*-1):Angle() )
		self.NPod3:SetPos( self:LocalToWorld(Vector(-120,200,-45)) )
		...
		local WD = constraint.Weld(self, self.NPod3, 0, 0, 0, true)
		--self.NPod3:SetParent( self )
	...
	if (self.NPod4 == nil or !self.NPod4:IsValid() or self.NPod4.Active) then
		...
		self.NPod4:SetAngles( (self:GetRight()*1):Angle() )
		self.NPod4:SetPos( self:LocalToWorld(Vector(-120,-200,-45)) )
		...
		local WD = constraint.Weld(self, self.NPod4, 0, 0, 0, true)
		--self.NPod4:SetParent( self )
