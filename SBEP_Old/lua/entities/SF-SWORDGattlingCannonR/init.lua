...
function ENT:Initialize()
	...
	self:SetModel( "models/Slyfo/swordgatright.mdl" ) 
	self:SetName("Gattling Right")
...
function ENT:Think()
	...
	if (self.Active == true or self.FTime > CurTime() ) then
		...
		local vStart = self:LocalToWorld(Vector(150,0,40))-- + (self:GetForward() * 20) + (self:GetUp() * BUp) + (self:GetRight() * BRi)
		local vForward = self:GetForward()
		
		local Bullet = {}
		Bullet.Num = 1
		Bullet.Src = self:LocalToWorld(Vector(150,0,40))
