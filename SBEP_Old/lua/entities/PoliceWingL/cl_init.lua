...
function ENT:Draw()
	...
	if self.Blip1 > 0 then
		...
		render.DrawSprite( self:GetPos() + self:GetRight() * 150 + self:GetForward() * 30, self.Blip1, self.Blip1, color )
		--self.Blip1 = self.Blip1 - 50
	end
	if self.Blip2 > 0 then
		local color = Color( 60, 60, 200, self.Blip2 )
		render.DrawSprite( self:GetPos() + self:GetRight() * -150 + self:GetForward() * 30, self.Blip2, self.Blip2, color )
