...
function ENT:Initialize()
	...
	self:SetModel( "models/Spacebuild/Nova/machuge.mdl" ) 
	self:SetName("Big MAC")
...
function ENT:Think()
	...
	if self.Charge > 0 then
		if CurTime() >= self.SpTime then
			...
			Sparky:SetPos(self:GetPos() + (self:GetForward() * math.Rand(-1347,120)) + (self:GetUp() * math.Rand(-125,100)) + (self:GetRight() * math.Rand(-90,90)) )
			Sparky:Spawn()
	...
	if CurTime() <= self.CDown then
		if CurTime() >= self.SmTime then
			...
			local effectdata = EffectData()
			effectdata:SetOrigin(self:GetPos() +  self:GetForward() * math.Rand(-1347,120))
			effectdata:SetStart(self:GetPos() +  self:GetForward() * math.Rand(-1347,120))
